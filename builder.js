...
async function worker({ progress, svgPath, options, renameFilter, template }) {
  ...
  const replaceClassName = (str) => str.replace(/class=/g, "className=");
  debugger;
  await fse.writeFile(absDestPath, replaceClassName(fileString));
}
...
export async function handler(options) {
  ...
  debugger;
  console.log(options.svgDir, options.glob);
...
const firstUpCase = (part) => part.charAt(0).toUpperCase() + part.substring(1);

handler({
  svgDir: "./src/icons/",
  outputDir: "./src/icon/",
  glob: "**/*.svg",
  renameFilter: (v) => firstUpCase(v.base.replace(".svg", "")) + "Icon" + ".js",
});
